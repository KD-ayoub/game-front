...
export default function Chat() {
  ...
  return (
    <main className="h-screen bg-[#0B0813] relative w-full max-w-[5120px] flex">
      <div className="h-[80px] w-[80px] sm:w-28 sm:h-28 md:w-64 md:h-64 lg:w-80 lg:h-80 xl:w-[480px] 
      ...
      >
        <div className={`chat ${NeuePlakFont.className} w-1265 h-864 bg-[#9b6363]`}>
          ...
          {option === "Friends" && !showConv && (
            <FriendsMessaged onChange={(value: boolean) => setShowConv(value)} />
          )}
          {option === "Channels" && !showConv && <ChannelMessaged />}
